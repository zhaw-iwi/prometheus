...
- [x] Milestone 128: Merge main documentation refresh into agents branch
...
- Published the repository documentation/assets state on `main`, including the README cleanup, new Valerian screenshots, and the pre-existing audio-tuning documentation archive move.
...
### Goal
Merge the main documentation and Codex-context refresh back into the `agents` branch while preserving the agents-branch application catalog and milestone history.

### What changed
- Merged main commit `94999aa` into `agents`, carrying the cleaned GitHub-facing README, refreshed `.agents/CONTEXT.md`, new Valerian screenshots, and audio-tuning archive move.
- Resolved `README.md` in favor of the current main README structure, then added an agents-branch catalog note for `tdsr.*`, `elderlycare.*`, and other event/experiment definitions.
- Resolved `PROJECT.md` by keeping the agents milestone trail, adding mainline milestones 115 and 116, and recording this merge as milestone 128.
- No Java runtime, Valerian client script, prompt, or agent-definition behaviour changes were intended in this merge.
...
### Next steps
1. Deploy this branch and repeat the audible test with the same paragraph.
...
### What changed
- Added `POST /demo/talktome/agents/{agentId}/speech`. The scoped application
  service acknowledges the observation with `REALTIME_SPEECH`, extracts the
  unchanged speech channel from the returned persisted `BehaviourPlan`, and
  only then invokes the synthesis gateway.
- Added an OpenAI Speech gateway for `/v1/audio/speech` with configurable
  ...
  Standard OpenAI requests use `gpt-4o-mini-tts` and MP3; voice and speed are
  validated server-side against the supported contract.
...
### Next steps
1. Deploy with `prometheus.talktome.speech.model=gpt-4o-mini-tts` and the Speech
   endpoint.
...
- Moved synthesis HTTP handling from `ScopedDemoController` into the dedicated
  ...
  `POST /demo/talktome/agents/{agentId}/speech`. The shared controller
  constructor, mappings, and MVC slices are restored to their pre-synthesis
  contract.
...
## 2026-09-17: Integrate Need for Speed into agents for Heroku testing

- Ordinary merge of features/needforspeed through f9fcffe into agents d0d3618.
  Kept agents deployment and catalog changes. Resolved PromptPolicy and its
  gesture test using the feature branch's combined-generation implementation
  and preserved top-level hand-sign normalization.
- Adapted four application unit-test gateways and the guessing-game replay's
  four paired generation responses. Kept expected speech, gesture, state and
  storage assertions. Three application SSE replay servers stop immediately at
  teardown, matching the framework fixture cleanup.
- Full merged Java suite: 398 tests, 96 classes, no failures/errors/skips, clean
  shutdown. Shared Node suites: 45 passed. Used only disposable MySQL schema
  prometheus_nfs_merge_b9643a17, mocks/loopback endpoints and a dummy provider key.
- User explicitly requested deploying agents to Heroku as the testing environment.
  The committed production profile selects Sol behaviour/nonverbal and Luna
  decisions/extraction/summary at none. Effective environment overrides and live
  response quality/performance remain deployment/trial observations.
...
- Merged feature commits f8d9c1b/13ba6ed into agents; all 459 merged Java cases
