...
Use the documentation of [PROMISE](https://github.com/zhaw-iwi/statefulconversation_java) to deploy existing conversational interactions. As part of this repository, you will find the following examples.
...
## 5 REST API for Multimodality
If a regulation system contains tanks, their current values can be altered via REST API. For example, this can be used to integrate a visual detection component that observes facial expressions or social situations. Such observations can then be provided to the regulation system which will adapt its tank levels accordingly.

There are currently two levels of manipulating tank levels.
- Given a regulation system (identified by its ID), update the system with an observation
- Given a particular tank (identified by its ID), increase or decrease its current value

Use the following REST API endpoints to integrate such components.
- **GET .../socialbehaviourregulation/regulationsystem/all**
- **PUT .../socialbehaviourregulation/regulationsystem/{id}/observe** with an observation in the request body
- **PUT .../socialbehaviourregulation/tank/{id}/increase**
- **PUT .../socialbehaviourregulation/tank/{id}/decrease**
