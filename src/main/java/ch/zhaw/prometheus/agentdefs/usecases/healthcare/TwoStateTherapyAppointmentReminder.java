...
public class TwoStateTherapyAppointmentReminder implements AgentDefinition {
    ...
    static final String PROMPT_INTRO_STATE = """
            ...
            For now, focus on who Valerian is, why he is present, and whether the person wants to hear
            more about Valerian or move on to the demonstration.

            Goals:
            - Make clear that Valerian is a socially intelligent digital agent in a healthcare care-center use case.
            - Explain that Valerian does not replace human care; he supports care staff and helps make
              small next steps less lonely.
            - Let the person choose between asking about Valerian in general and moving on to the
              therapy-reminder demonstration.
            - If the person asks about Valerian, answer warmly and briefly in ordinary spoken English.
            - If the person asks about PROMETHEUS or the system, explain only at a high level:
              it helps keep Valerian's task flow, context awareness, and behaviour structured and testable.
            ...
            Conversation style:
            For this intro state, two short sentences are allowed when explaining who Valerian is.
    ...
    static final String PROMPT_INTRO_STATE_STARTER = """
            ...
            "Hello, I am Valerian. I am not here to replace care; I am here to make your next step
    ...
    static final String PROMPT_INTRO_TO_THERAPY_REMINDER = """
            Decide whether the conversation should leave the Valerian introduction state
            ...
            Return false if the person asks who Valerian is, asks what Valerian can do,
            asks about PROMETHEUS, asks for more explanation, chooses to hear more about Valerian,
    ...
    static final String PROMPT_STATE_STARTER = """
            Say something like this, but not word for word:
            "Hello, I am Valerian. I wanted to gently remind you about your upcoming appointment.
    ...
    public static final String KEY = "usecases.healthcare.therapy_appointment_reminder_intro";

    public static Agent createAgentDefinition() {
        Storage storage = new Storage();
        HealthcareTherapyAppointmentContexts.preselect(storage, ThreadLocalRandom.current());

        State sessionFinal = new Final(
                "Valerian Use Cases Healthcare therapy reminder with intro complete",
                PROMPT_FINAL,
        ...
        Transition outerToFinal = new Transition(
                List.of(
                        ...
                        new StaticDecision(HealthcarePrompts.OUTER_STATE_TO_FINAL)),
                List.of(
        ...
        Agent agent = new Agent(
                "Valerian Use Cases Healthcare - Therapy Reminder (w. Intro)",
                "English healthcare care-center agent that introduces Valerian before the therapy reminder use case.",
                outerState,
                storage);
        agent.setInteractionProfile(HealthcareAgentFactory.healthcareCareProfile());
        return agent;
    }

    private static State introState(State therapyState) {
        PromptPolicy introPolicy = healthcarePromptPolicy(PROMPT_INTRO_STATE, PROMPT_INTRO_STATE_STARTER);
        State introState = new State("Valerian Use Cases Healthcare therapy reminder introduction", introPolicy, List.of());
    ...
    private static State therapyState(Storage storage, State sessionFinal) {
        ...
        applyHealthcareNonverbalPrompts(therapyPolicy);

        State therapyState = new State("Valerian Use Cases Healthcare therapy reminder use case", therapyPolicy, List.of());
        ...
        Transition reactToSocialContextChange = new Transition(
                List.of(
                        ...
                        new StaticDecision(HealthcarePrompts.SOCIAL_INTERJECTION_OPPORTUNITY)),
                List.of(),
                therapyState);
    ...
    private static PromptPolicy healthcarePromptPolicy(String prompt, String starter) {
        PromptPolicy policy = new PromptPolicy(prompt, starter, PromptPolicy.DEFAULT_SUMMARISE_PROMPT);
        applyHealthcareNonverbalPrompts(policy);
        return policy;
    }

    private static void applyHealthcareNonverbalPrompts(PromptPolicy policy) {
        policy.setNonVerbalPlanPrompt(HealthcarePrompts.NONVERBAL_PLAN);
