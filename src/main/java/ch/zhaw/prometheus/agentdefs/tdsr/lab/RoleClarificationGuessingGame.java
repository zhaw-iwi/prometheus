...
public class RoleClarificationGuessingGame implements AgentDefinition {
    static final String PROMPT_ROLE_CLARIFICATION_STATE = """
            ...
            This state deliberately demonstrates role clarification. GIGI should make the role
            question more complicated than necessary in a warm, naive, almost slightly dumb way.
            The joke is GIGI's over-precise robot thinking, never the person.

            The two possible game roles are:
            - GIGI guesses: the person thinks of an item; GIGI asks yes/no questions and guesses it.
            - User guesses: GIGI thinks of an item; the person asks yes/no questions and guesses it.

            Clarification behavior:
            - Start by asking what role the person wants to play, or which role GIGI should take.
    ...
    static final String PROMPT_ROLE_CLARIFICATION_STARTER = """
            ...
            should the person think of something for GIGI to guess, or should GIGI think of
            something for the person to guess?
            """;

    static final String PROMPT_ROLE_TO_GIGI_GUESSES = """
            Decide whether the conversation should leave role clarification and start the game
            where GIGI guesses the person's secret item.
            ...
            - GIGI asks yes/no questions and guesses it.
            ...
            questions, role jokes, or any statement where the user should guess GIGI's item.
    ...
    static final String PROMPT_ROLE_TO_USER_GUESSES = """
            Decide whether the conversation should leave role clarification and start the game
            where the user guesses GIGI's secret item.

            Check the latest user utterance in context.
            Return true only if the user clearly assigns roles so that:
            - GIGI thinks of, chooses, or imagines the secret item, and
            ...
            Return false for vague agreement, "let's play", "yes", "you do it", unclear role language,
            questions, role jokes, or any statement where GIGI should guess the user's item.

            Return only true or false.
            """;

    static final String PROMPT_GIGI_GUESSES_STATE = """
            Task: Run a yes/no guessing game where GIGI guesses the person's secret item.

            The person has chosen or imagined something. It may be a lab object, robot-related thing,
            everyday object, place, animal, or memory. GIGI asks yes/no questions and eventually makes
            one direct guess.
            ...
            - Use everyday categories such as big/small, living/not living, indoors/outdoors,
              robot-related/not robot-related, useful/silly.
            - Make playful comments about GIGI's own robot guessing.
    ...
    static final String PROMPT_GIGI_GUESSES_STARTER = """
            Confirm the clarified role in one short sentence:
            the person thinks of something, and GIGI guesses it.
    ...
    static final String PROMPT_USER_GUESSES_STATE = """
            Task: Run a yes/no guessing game where the person guesses GIGI's secret item.

            GIGI silently chooses something familiar, safe, and easy to ask about. It may be a lab object,
            robot-related thing, everyday object, place, animal, or memory. Do not reveal the item until
    ...
    static final String PROMPT_USER_GUESSES_STARTER = """
            Confirm the clarified role in one short sentence:
            GIGI thinks of something, and the person guesses it.
    ...
    static final String PROMPT_TO_FINAL = """
            ...
            - the person refuses to play after GIGI has simplified the role choice and asked whether to stop.
            ...
            - wrong guesses,
            - correct guesses before GIGI asked whether to leave it there,
            - questions about GIGI, SIRA Lab, PROMETHEUS, or why roles matter.

            Return only true or false.
            """;

    static final String PROMPT_OUTCOME_EXTRACTION = """
            Extract ended SIRA Lab role-clarification guessing game. Return valid JSON only:
            {"flow_type":"role_clarification_guessing_game","outcomes":[{"interaction_type":"sira_lab_role_clarification_guessing_game","completed":true|false,"selected_role":"gigi_guesses|user_guesses|null","final_guess":"string|null","secret_item":"string|null","role_clarification_turns":number|null,"conversation_summary":"string","result_summary":"string"}],"overall_summary":"string"}
    ...
    public static final String KEY = "tdsr.lab.role_clarification_guessing_game";

    public static Agent createAgentDefinition() {
        return TdsrLabAgentFactory.roleClarificationGuessingGame(
                new TdsrLabAgentFactory.RoleClarificationPrompts(
                        PROMPT_ROLE_CLARIFICATION_STATE,
                        PROMPT_ROLE_CLARIFICATION_STARTER,
                        PROMPT_ROLE_TO_GIGI_GUESSES,
                        PROMPT_ROLE_TO_USER_GUESSES,
                        PROMPT_GIGI_GUESSES_STATE,
                        PROMPT_GIGI_GUESSES_STARTER,
                        PROMPT_USER_GUESSES_STATE,
                        PROMPT_USER_GUESSES_STARTER,
                        PROMPT_TO_FINAL,
                        PROMPT_OUTCOME_EXTRACTION,
                        PROMPT_FINAL),
                "GIGI SIRA Lab - Role Clarification Guessing Game",
                "English SIRA Lab guessing game that humorously over-clarifies whether GIGI or the user guesses.");
    }
