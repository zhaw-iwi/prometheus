...
@Entity
public class LabRpsResultPolicy extends Policy {
    @ManyToOne(cascade = CascadeType.ALL, fetch = FetchType.EAGER)
    private Storage storage;

    protected LabRpsResultPolicy() {
    }

    public LabRpsResultPolicy(Storage storage) {
    ...
    public String describe() {
        return "Deterministic English SIRA Lab rock-scissor-paper result policy.";
    }
    ...
    private static String speech(JsonObject round, String winner) {
        ...
        return switch (winner) {
            ...
            case "user" -> "You win: " + reason(userSign, agentSign)
                    + ". My robot dignity is lightly dented. Again?";
