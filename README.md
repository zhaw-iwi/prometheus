...
- `tdsr.migros.*` - German Migros Appenzell TDSR station agents. `tdsr.migros.appenzell_general` is the broad station conversation agent for customers and employees, with compact social/weather context and speech plus gesture output. `tdsr.migros.appenzell_scene_2_menu_planner` and `tdsr.migros.appenzell_scene_3_checkout_reflection` are narrower filmed-scene agents for the menu-planner and checkout-reflection beats; they keep the same compact social/weather context and gesture-only output.
