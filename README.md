...
- `tdsr.migros.*` - German Migros Appenzell TDSR station agents, currently `tdsr.migros.appenzell_menu_planner`, an in-store menu-planning conversation agent that structures customer decisions, can use facial/social/weather context, and keeps Migros employees central to trust, regionality, and product confirmation while emitting full GIGI physical behaviour output.
